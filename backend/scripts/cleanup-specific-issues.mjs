...
async function cleanupSpecificIssues() {
  try {
    console.log('🔍 Buscando registros específicos problemáticos...\n')
    
    // Buscar registros con problemas específicos
    const problematicEntries = await prisma.vehicleEntry.findMany({
      where: {
        status: 'ingresado',
        exitDate: null,
        // Criterios más específicos para registros problemáticos
        OR: [
          // Sin órdenes de trabajo Y sin fotos Y sin control de llaves
          {
            AND: [
              { workOrders: { none: {} } },
              { photos: { none: {} } },
          ...
          // Ingresos muy antiguos sin actividad (más de 30 días)
          {
            AND: [
              { entryDate: { lt: new Date(Date.now() - 30 * 24 * 60 * 60 * 1000) } },
              { workOrders: { none: {} } }
            ]
          }
        ]
    ...
    if (DRY_RUN) {
      ...
      problematicEntries.forEach((entry, index) => {
        console.log(`\n[SIMULACIÓN] Se eliminaría: ${entry.entryCode}`)
      })
    } else {
